...
def _convert_to_lowercase(x):
    return x.lower()


def _tokenize_text(x):
    doc = nlp(x)
...
def _get_basic_features(df):
    if type(df) == pd.core.frame.DataFrame:
        df['text'] = df['text'].apply(lambda x: _lemmatize_text(x))
        df['text'] = df['text'].apply(lambda x: _convert_to_lowercase(x))
        df['text'] = df['text'].apply(lambda x: _tokenize_text(x))
